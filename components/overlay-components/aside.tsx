...
const Aside = ({ slug }: { slug: string | undefined }) => {
	...
	return (
		<>
			<button
				...
				className={`fixed right-4 top-14 z-50 p-2 bg-zinc-900/80 backdrop-blur-md border
			...
			<aside
				className={`right-0 top-12 fixed h-full w-72 bg-zinc-900 backdrop-blur-md border-l
         ...
         transition-transform duration-[275ms] ${toggleOpen ? "delay-100 translate-x-0 ease-in-out" : "translate-x-full ease-in"}`}
			>
				<button
					...
					className="p-2 border-b border-white/20 self-start  hover:text-yellow-500 transition-colors w-full"
				>
